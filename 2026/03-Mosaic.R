...
plot_dat$health <- case_when(plot_dat$health == "Vgood" ~ "Very good",
                             TRUE ~ paste(plot_dat$health))

health_levels <- c("Excellent",
                   "Very good",
                   "Good",
                     "Fair",
                     "Poor")
...
p_mosaic <- ggplot(plot_dat) +
  ...
  labs(title = paste("Education level vs self-reported health status \nacross", format(nrow(plot_dat), big.mark = " "), "participants (Dec 2011)"),
       x = "",
       y = "") +
  theme_mosaic() +
  theme(plot.title = element_text(size = 14, face = "bold", lineheight = 0.35),
